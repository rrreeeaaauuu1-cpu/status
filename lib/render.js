...
export const STYLE = {
  ...
  sutStroke: "#fffea1",
  sutTextColor: "#fffea1",
  // 불 (fire=none 이 아니면 최상단에 흰 불 프레임 애니메이션 재생)
  fireOpacity: 0.92,
  fireHeight: 560,               // 불 덩어리 높이(px)
  fireBaseBelow: 60,             // 바닥을 창 아래로 내려 밑부분(평평/자연) 숨김
  fireStep: 0.55,                // 타일 가로 간격 비율(작을수록 촘촘)
  fireDur: 1.4,                  // 플립북 한 바퀴(초, 크면 느리게)
};
...
// 흰 불 프레임 플립북(최상단): 13장을 크로스페이드로 부드럽게 이어 재생, 가로로 타일링
function buildFire(W, H, preview) {
  const N = FIRE_FRAMES.length;
  const dur = STYLE.fireDur;
  // 프레임 전환 키타임/불투명도 (삼각 크로스페이드 → 부드러운 전환, 마지막→처음 루프)
  const kt = [];
  ...
  // 프레임 이미지 defs (base64 는 한 번만 정의하고 <use> 로 재사용)
  let defs = "<defs>";
  for (let i = 0; i < N; i++) defs += `<image id="ff${i}" href="${FIRE_FRAMES[i]}" width="${FIRE_W}" height="${FIRE_H}"/>`;
  defs += "</defs>";

  const s = STYLE.fireHeight / FIRE_H;
  const cw = FIRE_W * s;
  const yTop = (H + STYLE.fireBaseBelow) - FIRE_H * s; // 바닥이 창 아래로 내려가도록
  const step = cw * STYLE.fireStep;
  const tiles = Math.ceil((W + cw) / step) + 1;

  let out = defs + `<g opacity="${STYLE.fireOpacity}">`;
  for (let t = 0; t < tiles; t++) {
    const xLeft = -cw * 0.35 + t * step;
    const phase = ((t * 0.618 * dur) % dur).toFixed(2); // 타일마다 위상 어긋나게
    let uses = "";
    if (preview) {
      // 정지 확인용: 타일마다 다른 프레임 하나만 표시
      uses = `<use href="#ff${t % N}" opacity="1"/>`;
    } else {
      for (let i = 0; i < N; i++) {
        uses += `<use href="#ff${i}" opacity="0">` +
          `<animate attributeName="opacity" values="${valuesFor(i)}" keyTimes="${keyTimes}" dur="${dur}s" begin="-${phase}s" repeatCount="indefinite" calcMode="linear"/></use>`;
      }
    }
    out += `<g transform="translate(${r(xLeft)},${r(yTop)}) scale(${r(s)})">${uses}</g>`;
  }
  return out + `</g>`;
}
