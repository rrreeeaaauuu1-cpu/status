...
function flameD(cx, baseY, hw, hh, tip, w1, w2, hole, hx, hy, hr) {
  const p =
    ...
    `C${r(cx - hw * 0.35 + w1)},${r(baseY - hh * 0.95)} ${r(cx + tip - hw * 0.16)},${r(baseY - hh * 1.06)} ${r(cx + tip)},${r(baseY - hh)} ` +
    `C${r(cx + tip + hw * 0.16)},${r(baseY - hh * 1.06)} ${r(cx + hw * 0.35 + w2)},${r(baseY - hh * 0.95)} ${r(cx + hw * 0.45 + w2)},${r(baseY - hh * 0.72)} ` +
...
function buildFire(W, H) {
  ...
  for (let i = 0; i < n; i++) {
    ...
    const tip0 = (rng() * 2 - 1) * hw * 0.7;
    const mk = (tip, w1, w2, hs, hy) => flameD(cx, baseY, hw, hh * hs, tip, w1, w2, hole, hx, hy, hr);
    const d1 = mk(tip0 - hw * 0.55, hw * 0.16, -hw * 0.14, 1.0, baseY - hh * 0.5);
    const d2 = mk(tip0 + hw * 0.55, -hw * 0.14, hw * 0.16, 0.84, baseY - hh * 0.4);
    const d3 = mk(tip0 + hw * 0.1, hw * 0.06, hw * 0.06, 1.12, baseY - hh * 0.58);
    out.push(
