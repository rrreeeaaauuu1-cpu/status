...
export const STYLE = {
  ...
  outlineRatio: 0.12,         // 외곽선 두께 = 글자크기 * 이 값 (0 이면 없음)
...
export const COORDS = {
  ...
  // 캐릭터 실루엣 안쪽 "출력 00%" (노멀 배경 제외하고 표시)
  power: { x: 45, cy: 505, font: 26, maxW: 185 },
};
...
function highlight(text, refX, refCy, refFont, refMaxW, scaleX, scaleY) {
  ...
  return `<text x="${r(x)}" y="${r(cy)}" ${common} fill="${STYLE.textColor}"${stroke}>${esc(text)}</text>`;
}
...
export function buildSvg(params, bg) {
  ...
  if (power) parts.push(highlight(power, COORDS.power.x, COORDS.power.cy, COORDS.power.font, COORDS.power.maxW, scaleX, scaleY));
