...
export const STYLE = {
  ...
  sutStroke: "#fffea1",
  sutTextColor: "#fffea1",
  // 불 (fire=none 이 아니면 최상단에 흰색 불꽃이 일렁임)
  fireColor: "#ffffff",
  fireOpacity: 0.92,
  fireCount: 9,                  // 불꽃 덩어리 개수
  fireWmin: 70,                  // 불꽃 반너비 min
  fireWmax: 130,                 // 불꽃 반너비 max
  fireHmin: 250,                 // 최소 높이
  fireHmax: 480,                 // 최대 높이
  fireDurMin: 1.6,               // 일렁임 주기(초, 클수록 느리고 부드럽게)
  fireDurMax: 2.8,
  fireHoleProb: 0.55,            // 내부 구멍 확률
};
...
export const COORDS = {
  ...
  date:    { x: 342, y: 188, font: 21, maxW: 205, anchor: "start" },
  ...
  power:   { x: 121,  y: 410, font: 40, maxW: 170, anchor: "start", color: "#d10d0e" }, // 실루엣 안 출력%
...
// 유기적 불꽃 하나의 path (밑변 → 불룩한 몸통 → 둥근 끝 → 밑변, 옵션 구멍)
function flameD(cx, baseY, hw, hh, tip, w1, w2, hole, hx, hy, hr) {
  const p =
    `M${r(cx - hw)},${r(baseY)} ` +
    `C${r(cx - hw * 1.05)},${r(baseY - hh * 0.32)} ${r(cx - hw * 0.55)},${r(baseY - hh * 0.5)} ${r(cx - hw * 0.45 + w1)},${r(baseY - hh * 0.72)} ` +
    `C${r(cx - hw * 0.4 + w1)},${r(baseY - hh * 0.92)} ${r(cx + tip - hw * 0.32)},${r(baseY - hh * 1.02)} ${r(cx + tip)},${r(baseY - hh)} ` +
    `C${r(cx + tip + hw * 0.32)},${r(baseY - hh * 1.02)} ${r(cx + hw * 0.4 + w2)},${r(baseY - hh * 0.92)} ${r(cx + hw * 0.45 + w2)},${r(baseY - hh * 0.72)} ` +
    `C${r(cx + hw * 0.55)},${r(baseY - hh * 0.5)} ${r(cx + hw * 1.05)},${r(baseY - hh * 0.32)} ${r(cx + hw)},${r(baseY)} Z`;
  if (!hole) return p;
  const c = 0.5523 * hr;
  return p +
    ` M${r(hx)},${r(hy - hr)} ` +
    `C${r(hx + c)},${r(hy - hr)} ${r(hx + hr)},${r(hy - c)} ${r(hx + hr)},${r(hy)} ` +
    `C${r(hx + hr)},${r(hy + c)} ${r(hx + c)},${r(hy + hr)} ${r(hx)},${r(hy + hr)} ` +
    `C${r(hx - c)},${r(hy + hr)} ${r(hx - hr)},${r(hy + c)} ${r(hx - hr)},${r(hy)} ` +
    `C${r(hx - hr)},${r(hy - c)} ${r(hx - c)},${r(hy - hr)} ${r(hx)},${r(hy - hr)} Z`;
}

// 흰색 불꽃 레이어(최상단): 유기적 덩어리가 느리고 부드럽게 일렁이며 UI를 가림
function buildFire(W, H) {
  const rng = mulberry32(hashSeed("overlapfire"));
  const n = STYLE.fireCount;
  const baseY = H + 16;
  const out = [];
  const ks = "0.42 0 0.58 1";  // easeInOut → 부드러운 일렁임
  for (let i = 0; i < n; i++) {
    const cx = (i + 0.5) * (W / n) + (rng() * 2 - 1) * 26;
    const hw = STYLE.fireWmin + rng() * (STYLE.fireWmax - STYLE.fireWmin);
    const hh = STYLE.fireHmin + rng() * (STYLE.fireHmax - STYLE.fireHmin);
    const dur = (STYLE.fireDurMin + rng() * (STYLE.fireDurMax - STYLE.fireDurMin)).toFixed(2);
    const begin = "-" + (rng() * dur).toFixed(2);  // 시작 위상 분산
    const hole = rng() < STYLE.fireHoleProb;
    const hr = 15 + rng() * 18;
    const hx = cx + (rng() * 2 - 1) * hw * 0.3;
    const tip0 = (rng() * 2 - 1) * hw * 0.5;
    const mk = (tip, w1, w2, hs, hy) => flameD(cx, baseY, hw, hh * hs, tip, w1, w2, hole, hx, hy, hr);
    const d1 = mk(tip0 - hw * 0.38, hw * 0.12, -hw * 0.12, 1.0, baseY - hh * 0.5);
    const d2 = mk(tip0 + hw * 0.38, -hw * 0.12, hw * 0.12, 0.86, baseY - hh * 0.42);
    const d3 = mk(tip0, hw * 0.05, hw * 0.05, 1.1, baseY - hh * 0.58);
    out.push(
      `<path fill="${STYLE.fireColor}" fill-opacity="${STYLE.fireOpacity}" fill-rule="evenodd" d="${d1}">` +
      `<animate attributeName="d" values="${d1};${d2};${d3};${d1}" dur="${dur}s" begin="${begin}s" repeatCount="indefinite" ` +
      `calcMode="spline" keyTimes="0;0.34;0.67;1" keySplines="${ks};${ks};${ks}"/></path>`
    );
  }
  return `<g>${out.join("")}</g>`;
}
...
export function buildSvg(params, assets) {
  ...
  if (boxItems.length) {
    ...
    boxItems.forEach((item, i) => {
      ...
      // 글리치는 부상박스(glitch:true)만. 봉합박스는 애니메이션 없음.
  ...
  // 7) 불 (최상단 레이어) — fire 가 none/빈값이 아니면 표시
  if (params.fire && String(params.fire).toLowerCase() !== "none") {
    parts.push(buildFire(W, H));
  }
