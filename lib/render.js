...
export function buildSvg(params, assets) {
  ...
  // 8) 킬 카운트 (도서관 모드 전용, 최상단): 빨간 KOTRA, 랜덤 위치, 빠른 떨림 + 좌우 기울임
  const killV = (params.kill == null ? "" : String(params.kill)).trim();
  if (lib && killV && killV.toLowerCase() !== "none") {
    const rng = mulberry32(hashSeed("kill:" + killV));
    const px = r(130 + rng() * (W - 260));
    const py = r(70 + rng() * (H - 140));
    const A = STYLE.killTiltDeg, tr = STYLE.killTremble;
    const tilt = `<animateTransform attributeName="transform" type="rotate" additive="sum" values="${-A} 0 0;${A} 0 0;${-A} 0 0" dur="${STYLE.killTiltDur}s" repeatCount="indefinite" calcMode="spline" keyTimes="0;0.5;1" keySplines="0.4 0 0.6 1;0.4 0 0.6 1"/>`;
