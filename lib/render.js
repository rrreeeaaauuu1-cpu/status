...
export const STYLE = {
  ...
  fireOpacity: 1,                // 1 = 불투명
  fireCover: 1.15,              // 각 영역을 덮는 배율(1=딱맞게, 클수록 크게)
  fireDur: 2.4,                 // 플립북 한 바퀴(초) — 크면 느리게
...
function buildFire(W, H, preview) {
  ...
  for (let k = 0; k < N; k++) sp.push("0.45 0 0.55 1"); // easeInOut → 부드러운 전환
  const keyTimes = kt.join(";");
  const keySplines = sp.join(";");
  const valuesFor = (i) => {
    const v = [];
    ...
    if (preview) {
      ...
      for (let i = 0; i < N; i++) {
        uses += `<use href="#ff${i}" opacity="0">` +
          ...
          `dur="${dur}s" begin="-${phase}s" repeatCount="indefinite" calcMode="spline" keySplines="${keySplines}"/></use>`;
      }
    }
