...
export const STYLE = {
  ...
  bgScrollDur: 6.4,              // 배경 격자 왼쪽 스크롤 한 바퀴(초)
  // 박스 안쪽
  boxGradFrom: "#fc0303",        // 좌상단 색
  boxGradTo: "#000000",          // 우하단 색
  boxGradOpacity: 0.56,          // 박스 배경 투명도
  boxGridSize: 27,               // 박스 격자 크기(작을수록 촘촘)
  boxGridTilt: 5,                // 박스 격자 기울기(도, 오른쪽)
  boxScrollDur: 5.7,             // 박스 격자 오른쪽 스크롤(초)
  boxFlickerDur: 0.45,           // 박스 배경 점멸 속도(초)
  // 부상 박스
  dmgFill: "#7d1f22",
  dmgFillOpacity: 0.5,
  dmgStroke: "#d10d0e",
...
export function buildSvg(params, assets) {
  ...
  for (const key of Object.keys(COORDS.boxes)) {
    ...
    parts.push(
      ...
      `<animate attributeName="opacity" values="${of};${r(of * 0.32)};${r(of * 0.85)};${r(of * 0.45)};${of}" dur="${STYLE.boxFlickerDur}s" repeatCount="indefinite"/>` +
      `</rect>` +
      // 격자 = 점멸 없이 오른쪽 스크롤
