...
export function buildSvg(params, assets) {
  ...
  const boxItems = [
    ...splitList(params.dmg).map((t) => ({ text: t, fill: STYLE.dmgFill, stroke: STYLE.dmgStroke, tc: STYLE.dmgTextColor, glitch: true })),
    ...splitList(params.sut).map((t) => ({ text: t, fill: STYLE.sutFill, stroke: STYLE.sutStroke, tc: STYLE.sutTextColor, glitch: false })),
  ];
  if (boxItems.length) {
