 ...
 *   5) 텍스트 값들
 *   6) 부상 박스(랜덤 반투명, 70% 겹침/30% 랜덤, 순차 등장)
 *
...
export const STYLE = {
  fontFamily: "OverlapKR",       // 아래 @font-face 에 심는 이름 (font.otf = 도현체)
  textColor: "#f2d9d9",
  gridColor: "#fc0303",
  gridOpacity: 0.6,
  gridSize: 23,                  // 배경 격자 (촘촘)
  gridTilt: -5,                  // 배경 격자 기울기(도, 왼쪽)
  bgScrollDur: 9,                // 배경 격자 왼쪽 스크롤 한 바퀴(초)
  // 박스 안쪽
  boxGradFrom: "#fc0303",        // 좌상단 색
  boxGradTo: "#000000",          // 우하단 색
  boxGradOpacity: 0.75,
  boxGridSize: 46,               // 박스 격자 (배경의 절반 밀도)
  boxGridTilt: 5,                // 박스 격자 기울기(도, 오른쪽)
  boxScrollDur: 8,               // 박스 격자 오른쪽 스크롤(초)
  boxFlickerDur: 0.45,           // 박스 점멸 속도(초)
  // 부상 박스
  dmgFill: "#7d1f22",
  dmgFillOpacity: 0.5,
  dmgStroke: "#d10d0e",
...
export const COORDS = {
  ...
  faction: { x: 453, y: 62, font: 20, maxW: 150, anchor: "middle" },   // FACTION 박스 안(소속)
  date:    { x: 312, y: 197, font: 19, maxW: 205, anchor: "start" },
  time:    { x: 527, y: 216, font: 21, maxW: 150, anchor: "start" },
  location:{ x: 527, y: 248, font: 21, maxW: 175, anchor: "start" },
  turn:    { x: 875, y: 248, font: 21, maxW: 90,  anchor: "middle" },
  nest:    { x: 400, y: 372, font: 24, maxW: 130, anchor: "middle" },
  power:   { x: 74,  y: 310, font: 30, maxW: 170, anchor: "start", color: "#d10d0e" }, // 실루엣 안 출력%

  // 목록(2열 x 6줄)
  inventory: { cols: [548, 740], rowY: [74, 92, 110, 128, 146, 164], font: 16, maxW: 185 },
  relationship: { cols: [548, 762], rowY: [318, 344, 370, 396, 422, 448], font: 16, maxW: 210 },

  // 박스 사각형(반투명 점멸 격자 들어갈 영역) : {x,y,w,h}
  boxes: {
    faction:      { x: 372, y: 12,  w: 164, h: 150 },
    inventory:    { x: 540, y: 14,  w: 385, h: 171 },
    relationship: { x: 512, y: 272, w: 483, h: 217 },
  },
};

// 부상 박스가 "가급적 피할" UI 영역(빈 곳 우선)
const AVOID_ZONES = [
...
export function buildSvg(params, assets) {
  ...
  parts.push(
    ...
    `<path d="M${bg2} 0 L0 0 0 ${bg2}" fill="none" stroke="${STYLE.gridColor}" stroke-opacity="${STYLE.gridOpacity}" stroke-width="1"/></pattern>`
  );
  parts.push(
    `<linearGradient id="boxgrad" x1="0" y1="0" x2="1" y2="1">` +
    `<stop offset="0" stop-color="${STYLE.boxGradFrom}"/><stop offset="1" stop-color="${STYLE.boxGradTo}"/></linearGradient>`
  );
  // 박스 클립
  for (const key of Object.keys(COORDS.boxes)) {
    const b = COORDS.boxes[key];
    parts.push(`<clipPath id="clip_${key}"><rect x="${b.x}" y="${b.y}" width="${b.w}" height="${b.h}"/></clipPath>`);
  }
  parts.push(`</defs>`);
  ...
  // 3) 박스 안쪽: 대각선 그라데이션 + 격자(오른쪽 기울기/오른쪽 스크롤/점멸)
  for (const key of Object.keys(COORDS.boxes)) {
    const b = COORDS.boxes[key];
    const bcx = b.x + b.w / 2, bcy = b.y + b.h / 2;
    parts.push(
      `<g clip-path="url(#clip_${key})">` +
      `<rect x="${b.x}" y="${b.y}" width="${b.w}" height="${b.h}" fill="url(#boxgrad)" opacity="${STYLE.boxGradOpacity}"/>` +
      ...
      `<animate attributeName="opacity" values="0.6;0.2;0.55;0.25;0.6" dur="${STYLE.boxFlickerDur}s" repeatCount="indefinite"/>` +
      `</rect></g></g>`
    );
  }
  ...
  // 인벤토리 / 인연 (2열 x 6줄)
  const twoCol = (listStr, c) => {
    const items = splitList(listStr).slice(0, c.rowY.length * 2);
    items.forEach((it, i) => {
      if (!it) return;
      const col = i < c.rowY.length ? 0 : 1;
      const row = i % c.rowY.length;
      parts.push(textEl(it, c.cols[col], c.rowY[row], c.font, { maxW: c.maxW, anchor: "start" }));
    });
  };
  twoCol(params.inv, COORDS.inventory);
  twoCol(params.rel, COORDS.relationship);
    ...
    for (let i = 0; i < dmg.length; i++) {
      ...
      for (let tries = 0; tries < 45; tries++) {
        ...
        if (overlap) {
          ...
          x = base.x + (rng() * 2 - 1) * bw * 0.6;
          y = base.y + (rng() * 2 - 1) * bh * 0.75;
        } else {
          x = rng() * (W - bw);
          y = rng() * (H - bh);
        }
