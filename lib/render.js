...
export function buildSvg(params, assets) {
  ...
  for (const key of Object.keys(COORDS.boxes)) {
    ...
    parts.push(
      ...
      `<rect x="${b.x}" y="${b.y}" width="${b.w}" height="${b.h}" fill="${STYLE.boxTint}" opacity="${STYLE.boxTintOpacity}"/>` +
      `<g transform="rotate(${STYLE.gridTilt} ${bcx} ${bcy})">` +
      `<rect x="${b.x - 80}" y="${b.y - 80}" width="${b.w + 160}" height="${b.h + 160}" fill="url(#grid)">` +
      `<animate attributeName="opacity" values="0.6;0.15;0.55;0.2;0.6" dur="${STYLE.boxFlickerDur}s" repeatCount="indefinite"/>` +
      `</rect></g></g>`
    );
  }
