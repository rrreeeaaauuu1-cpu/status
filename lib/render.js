...
export const STYLE = {
  ...
  fireHmin: 170,                 // 최소 높이
  fireHmax: 470,                 // 최대 높이
  fireDurMin: 0.22,              // 일렁임 속도(초, 작을수록 빠름)
  fireDurMax: 0.4,
};
...
// 흰색 불꽃 레이어(최상단): 여러 불꽃 혀가 빠르게 일렁이며 UI를 가림
function buildFire(W, H) {
  const rng = mulberry32(hashSeed("overlapfire"));
  const n = STYLE.fireCount;
  const baseY = H + 12;
  const out = [];
  // 불꽃 혀 경로(밑변 → 흔들리는 끝점 → 밑변)
  const tongue = (cx, hw, hh, tx) =>
    `M${r(cx - hw)},${r(baseY)} ` +
    `C${r(cx - hw * 0.7)},${r(baseY - hh * 0.45)} ${r(cx - hw * 0.5)},${r(baseY - hh * 0.78)} ${r(cx + tx)},${r(baseY - hh)} ` +
    `C${r(cx + hw * 0.5)},${r(baseY - hh * 0.78)} ${r(cx + hw * 0.7)},${r(baseY - hh * 0.45)} ${r(cx + hw)},${r(baseY)} Z`;
  for (let i = 0; i < n; i++) {
    const cx = (i + 0.5) * (W / n) + (rng() * 2 - 1) * 20;
    const hw = 46 + rng() * 46;
    const hh = STYLE.fireHmin + rng() * (STYLE.fireHmax - STYLE.fireHmin);
    const tx0 = (rng() * 2 - 1) * hw * 0.4;
    const dur = (STYLE.fireDurMin + rng() * (STYLE.fireDurMax - STYLE.fireDurMin)).toFixed(2);
    const begin = (rng() * 0.4).toFixed(2);
    const d1 = tongue(cx, hw, hh, tx0 - hw * 0.4);
    const d2 = tongue(cx, hw, hh * 0.85, tx0 + hw * 0.4);
    const d3 = tongue(cx, hw, hh * 1.08, tx0);
    out.push(
      `<path fill="${STYLE.fireColor}" fill-opacity="${STYLE.fireOpacity}" d="${d1}">` +
      `<animate attributeName="d" values="${d1};${d2};${d3};${d1}" dur="${dur}s" begin="${begin}s" repeatCount="indefinite"/></path>`
    );
  }
  return `<g>${out.join("")}</g>`;
}
