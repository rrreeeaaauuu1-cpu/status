...
export const COORDS = {
  status: { x: 178, cy: 154, font: 36, maxW: 235 },
  state:  { x: 582, cy: 154, font: 38, maxW: 200 },
  // DAMAGE: 왼쪽 칸(1~5) + 오른쪽 칸(6~10)
  //   x  / maxW  = 왼쪽 칸 위치/폭
  //   x2 / maxW2 = 오른쪽 칸 위치/폭  (rows 는 두 칸 공통)
  damage: { x: 322, x2: 542, font: 38, maxW: 240, maxW2: 240, rows: [277, 327, 377, 427, 477] },
  inventory: { x: 788, font: 38, maxW: 212, rows: [219, 284, 350, 416, 483] },
 
  // 하단 FACTION(소속) 값 — 라벨 오른쪽
  faction: { x: 448, cy: 549, font: 38, maxW: 300 },
 
  // 캐릭터 실루엣 안쪽 출력% (노멀 배경 제외하고 표시)
  power: { x: 80, cy: 460, font: 50, maxW: 185, color: "#d10d0e" },
};
 ...
 *   damage/inventory 는 "." 로 구분된 문자열, power 는 이미 완성된 문자열(예: "73%") 또는 빈 값
 *   damage 는 최대 10개 — 앞 5개는 왼쪽 칸, 다음 5개는 오른쪽 칸
 * bg:     { dataUri, w, h }
 */
export function buildSvg(params, bg) {
  ...
  // DAMAGE: 최대 10개 (0~4 왼쪽 칸, 5~9 오른쪽 칸)
  const dRows = COORDS.damage.rows.length; // 5
  damage.slice(0, dRows * 2).forEach((t, i) => {
    if (!t) return;
    const first = i < dRows;
    const dx = first ? COORDS.damage.x : COORDS.damage.x2;
    const dmaxW = first ? COORDS.damage.maxW : COORDS.damage.maxW2;
    const dy = COORDS.damage.rows[i % dRows];
    parts.push(highlight(t, dx, dy, COORDS.damage.font, dmaxW, scaleX, scaleY));
  });
