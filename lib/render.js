...
export const COORDS = {
  status: { x: 185, cy: 148, font: 27, maxW: 235 },
  state:  { x: 582, cy: 148, font: 27, maxW: 200 },
  damage: { x: 322, font: 25, maxW: 240, rows: [277, 327, 377, 427, 477] },
  inventory: { x: 788, font: 25, maxW: 212, rows: [219, 282, 345, 408, 471] },
};
...
function highlight(text, refX, refCy, refFont, refMaxW, scaleX, scaleY) {
  ...
  return (
    `<rect x="${r(x)}" y="${r(boxY)}" width="${r(boxW)}" height="${r(boxH)}" rx="${STYLE.boxRadius}" fill="${STYLE.boxFill}"/>` +
    `<text x="${r(x + padX)}" y="${r(cy)}" font-family="${STYLE.fontFamily}" font-size="${r(font)}" font-weight="${STYLE.fontWeight}" fill="${STYLE.textColor}" dominant-baseline="central" text-anchor="start">${esc(text)}</text>`
  );
}

/**
 * params: { status, state, damage, inventory }  (damage/inventory 는 "." 로 구분된 문자열)
 * bg:     { dataUri, w, h }
 */
