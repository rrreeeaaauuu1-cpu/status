...
export const STYLE = {
  ...
  textColor: "#fc0303",          // 기본(OVERLAP) 글자색
  libTextColor: "#e6d6a2",       // 도서관(LIBR) 모드 글자색
  // 킬 카운트(도서관 모드 전용, 최상단): 빨간 KOTRA, 랜덤 위치, 빠른 떨림 + 좌우 기울임
  killColor: "#fc0303",
  killFont: 41,                  // 20% 크게
...
// 화면 전체 빨간 노이즈 지직(간헐적). 매 요청마다 다른 불규칙 패턴(연속/대기 섞임).
function buildGlitch(W, H) {
  const T = STYLE.glitchCycle;
  const slots = 54;
  const kt = [], vals = [];
  let on = false;
  for (let i = 0; i < slots; i++) {
    kt.push((i / slots).toFixed(4));
    const p = on ? 0.5 : 0.06;               // 켜져있으면 이어질 확률↑(연속), 아니면 낮음(대기)
    on = Math.random() < p;
  ...
  const [cr, cg, cb] = STYLE.glitchColor;
  return (
    `<defs><filter id="rednoise" x="0" y="0" width="100%" height="100%">` +
    `<feTurbulence type="fractalNoise" baseFrequency="${STYLE.glitchFreq}" numOctaves="2" seed="5" stitchTiles="stitch" result="n">` +
    `<animate attributeName="seed" values="${seed.join(";")}" dur="0.12s" repeatCount="indefinite" calcMode="discrete"/></feTurbulence>` +
    `<feColorMatrix in="n" type="matrix" values="0 0 0 0 ${cr} 0 0 0 0 ${cg} 0 0 0 0 ${cb} 4 0 0 0 -2"/>` +
    `</filter></defs>` +
    `<rect width="${W}" height="${H}" filter="url(#rednoise)" opacity="0">` +
    `<animate attributeName="opacity" values="${vals.join(";")}" keyTimes="${kt.join(";")}" dur="${T}s" repeatCount="indefinite" calcMode="discrete"/></rect>`
  );
}
...
export function buildSvg(params, assets) {
  ...
  const lib = !!params.library;                       // 도서관(LIBR) 모드
  const txt = lib ? STYLE.libTextColor : STYLE.textColor;
  ...
  // 킬 카운트용 별도 폰트(KOTRA) — 도서관 모드에서 본문(아리따부리)과 다르게
  if (assets.killFontDataUri) {
    parts.push(`<style>@font-face{font-family:'KillKR';src:url('${assets.killFontDataUri}') format('${assets.killFontFormat || "opentype"}');}</style>`);
  }
  parts.push(
  ...
  // OVERLAP 모드에서만: 비네트 + 격자 + 박스 점멸. (LIBR 모드는 도서관 이미지가 전체 배경)
  if (!lib) {
  ...
  // 4) UI 오버레이 (A/B/C/D — OVERLAP은 투명 프레임, LIBR은 도서관 전체 배경)
  ...
  if (params.fire && String(params.fire).toLowerCase() !== "none") {
    parts.push(buildFire(W, H, params._preview));
  }

  // 8) 킬 카운트 (도서관 모드 전용, 최상단): 빨간 KOTRA, 매번 랜덤 위치, 빠른 떨림 + 좌우 기울임
  ...
  if (lib && killV && killV.toLowerCase() !== "none") {
    ...
    parts.push(
      ...
      `<text x="0" y="0" font-family="KillKR" font-size="${STYLE.killFont}" font-weight="700" fill="${STYLE.killColor}" text-anchor="middle" dominant-baseline="central">${esc("KILL " + killV)}</text>` +
      `</g>`
    );
  }

  // 9) 도서관 전체 빨간 노이즈 지직(간헐적, 최상단)
  if (lib) parts.push(buildGlitch(W, H));
