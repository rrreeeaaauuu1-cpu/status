...
function glowText(t, x, y, font, { anchor = "middle", maxW } = {}) {
  const f = maxW ? fitFont(t, font, maxW) : font;
  const common =
  ...
  return (
    // 글로우(빨간 번짐) — 빠르게 깜빡임
    `<text ${common} fill="${STYLE.glowColor}" filter="url(#txglow)">` +
...
export function buildSvg(params, assets) {
  ...
  if (lib && killV && killV.toLowerCase() !== "none") {
    ...
    parts.push(
      ...
      glowText("KILL " + killV, 0, 0, STYLE.killFont, { anchor: "middle" }) +
      `</g>`
    );
  }
